...
- 「歌单 → 我的收藏」＝ 洛雪的 `loveList`；爱心 / 行菜单可取消收藏，改动会同步
  （收藏列表在歌单页的内置歌单里，不在「我的」页）。
...
非平台歌曲——`source === 'local'` 的本地歌、WebDAV 云端歌、以及没有 `musicInfo` 的条目
——**不参与同步**：上行前会被过滤掉（本地歌与云端歌存在各自的侧表里，见
`core/local/SideTable`），所以本地库不会被这些条目污染，服务端也不会收到同步协议不认识的
东西。反过来，下行时它们也不会被覆盖掉。
