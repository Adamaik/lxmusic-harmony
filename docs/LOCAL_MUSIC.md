...
> 删除（单条 / 长按进多选批量）、混进播放队列、加进歌单与收藏（非同步侧表）、
> 同步设置页注明本地歌会被抛弃。
...
界面侧：播放页的音质标签对本地歌显示「本地」（这颗标签现在是个钮：本机文件没有档可切，
点开的面板只会说明这一点），下载钮弹「已经在本机了」；
...
| 空间占用 | 不做 LRU（导入是用户显式行为）；「我的 - 本地音乐」顶部显示「N 首 · X MB」，移除走长按多选（或右上角「选择」）→ 全选 → 移除 |
...
- **P0 已完成**：导入（文件管理的音频聚合视图）→ 沙箱副本
  ...
  「我的」页「本地音乐」分段页（导入钮 + 进度 + 列表 + 单条删除 + 长按多选批量移除）→ `playAt` 本地直路
