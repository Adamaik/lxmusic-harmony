...
- **我的页**：播放历史 / 下载管理 / 本地音乐 / 云端音乐四个列表都支持**长按任意一行进多选**，
  可批量收藏 / 加入歌单 / 加入播放列表，并按列表性质批量删（历史删记录、下载连文件删、
  本地音乐只移除应用内副本）；云端音乐不提供删除（文件在服务器上）。
  标题右侧原有的「清空」钮去掉了 —— 全选再删是同一件事，还省掉每个分段一套「清空什么」的规则
  （见 `docs/UI_REPLICA.md`）
- **本地音乐**：导入设备里的音频文件 —— 「导入音乐」直接拉起文件管理的音频聚合视图
  （一屏列出全机音频，可全选，这是手机端三方应用唯一接近「扫描本机音乐」的能力；
  三方应用不能自己扫全盘，原因见 `docs/LOCAL_MUSIC.md`）；列表支持长按多选批量移除。
  导入后拷一份进沙箱，可离线播放、
...
- **深浅模式**：跟随系统（也可在外观页钉死浅色 / 深色）；界面颜色全部是
  `resources/base` 与 `resources/dark` 下的同名资源 token，切模式即整体换色
- **设置**：播放设置 / 下载设置 / 缓存管理 / 音源管理 / 同步设置 / 外观设置
  （外观页管深浅模式、HDS 玻璃材质档位、首页流光、播放页封面圆角，见 `docs/UI_REPLICA.md`）
- **大屏**：折叠屏展开态 / 平板（`deviceTypes` 含 `tablet`）—— 底部两条悬浮栏的宽度、
  内容列的最大宽度都按窗口宽度算，旋转 / 折叠 / 分屏后自动重排
